...
namespace CartoonFX
{
    ...
    public class CFXR_ParticleText : MonoBehaviour
    {
        ...
        [SerializeField] float MinletterSpacing = 0.44f;
        [SerializeField] float MaxletterSpacing = 0.44f;
        ...
        )
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                if (char.IsWhiteSpace(text[i]))
                {
                    if (i > 0)
                    {
                        float letterSpacing = Random.Range(MinletterSpacing, MaxletterSpacing);
